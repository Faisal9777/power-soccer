...
#func _spawn_players_from_roster() -> void:
	...
	#for pid in match_config.roster.keys():
		#var info := match_config.roster[pid]
...
#func _spawn_ball_at(pos: Vector3) -> void:
	...
	#if ball == null:
		#var ball_scene: PackedScene = GameState.ball_scene
		#ball = ball_scene.instantiate()
		#ball.name = "Ball"
		#ball_holder.add_child(ball, true)
	## Reset physics then move
	#ball.call("reset_state")
...
#func _do_kickoff_positions() -> void:
	...
	#for p in players_root.get_children():
		...
		#var team := int(p.team)
		#var idx  := p.get("team_index") if p.has_method("get") else 0
